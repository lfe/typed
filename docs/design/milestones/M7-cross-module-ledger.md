...
| X-9 | **Regression + process:** full M0–M6 suites pass; exact assertions; CT in LFE; `make check` clean; CI green (0 skipped). | full CT + Rust green; `make check` exit 0 | serious | M0–M6 | **done** | `make check` exit 0: 82 Rust tests, 74 CT tests (10 adt + 6 chain + 9 cross_module + 11 dogfood + 6 matching + 12 records + 15 runtime + 5 typecheck), 0 skipped | |
