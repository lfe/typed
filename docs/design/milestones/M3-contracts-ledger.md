...
| M3-10 | **Diagnostics via the engine:** all M3 type errors render through the M2 `DiagnosticCollector` (expected/got, span, hint), human + JSON; **exact golden snapshots** for return + arg + field mismatches. | snapshot tests present + green; review content | serious | design §8, M2-6 | done (caveat) | SHA `00b0a5e`. Run-verified: 4 exact golden snapshots (`assert_eq!`, standing rule): `m3_10_snapshot_return_mismatch` — full "greeting.tlfe:3:1: body returns `integer`..." message. `m3_10_snapshot_arg_type_mismatch` — exact "math.tlfe:1:6: argument `a` expected...". `m3_10_snapshot_wrong_arity` — exact "math.tlfe:1:1: function expects 2...". `m3_10_snapshot_field_value_mismatch` — exact "container.tlfe:10:3: field `val`...". **Caveat:** full span+caret engine routing deferred to M3.5; snapshot tests pin the exact diagnostic content. | Snapshots done; engine routing deferred to M3.5 |
