...
| F-11 | CI harness skeleton runs the M0 suite with a **backend-matrix axis** present (tagged-tuple path wired; native-record axis stubbed). | CI config file present + one green run; matrix axis visible | polish | design §9 | **done** | SHA `a3d0834`. `.github/workflows/ci.yml` present; matrix `otp:['28']` / `repr-backend:['tagged-tuple']`; native-record axis stubbed. **Green CI run confirmed 2026-06-06** (Duncan) — re-entry condition met. The green run (cargo test + rebar3 ct on OTP 28) ALSO converts M0's & M1's CDC read-verified rows → independently run-verified. | closed: green run achieved |
