...
(defun x3_qualified_cross_module (config)
  (let* ((priv-dir (proplists:get_value 'priv_dir config))
         ...
         (orders-forms (check-and-decode config "cross_module" "orders.lfet")))
    (case (typed_driver:compile_forms orders-forms "orders.lfet" priv-dir)
      (`#(ok orders ,orders-bin)
       ...
       (let ((`#(module orders) (code:load_binary 'orders "orders.beam" orders-bin)))
         ...
         (let ((web-forms (check-and-decode config "cross_module" "orders_web.lfet")))
           (case (typed_driver:compile_forms web-forms "orders_web.lfet" priv-dir)
  ...
  (let* ((priv-dir (proplists:get_value 'priv_dir config))
    ...
    (case (typed_driver:compile_forms orders-forms "orders.lfet" priv-dir)
      (`#(ok orders ,orders-bin)
       (code:purge 'orders)
       (let ((`#(module orders) (code:load_binary 'orders "orders.beam" orders-bin)))
         (let ((acc-forms (check-and-decode config "cross_module" "orders_web_accessor.lfet")))
           (case (typed_driver:compile_forms acc-forms "orders_web_accessor.lfet" priv-dir)
  ...
  (let* ((priv-dir (proplists:get_value 'priv_dir config))
    ...
    (case (typed_driver:compile_forms orders-forms "orders.lfet" priv-dir)
      (`#(ok orders ,orders-bin)
       (code:purge 'orders)
       (let ((`#(module orders) (code:load_binary 'orders "orders.beam" orders-bin)))
         (let ((import-forms (check-and-decode config "cross_module" "orders_web_import.lfet")))
           (case (typed_driver:compile_forms import-forms "orders_web_import.lfet" priv-dir)
...
(defun x5_unknown_module (config)
  (let* ((checker-bin (proplists:get_value 'checker_bin config))
         ...
         (fixture (filename:join (list fixture-dir "cross_module" "bad_unknown_module.lfet")))
...
(defun x5_unknown_type (config)
  (let* ((checker-bin (proplists:get_value 'checker_bin config))
         (fixture-dir (proplists:get_value 'fixture_dir config))
         (fixture (filename:join (list fixture-dir "cross_module" "bad_unknown_type.lfet")))
         (`#(,exit-code ,output) (run-checker-raw checker-bin fixture)))
    (case exit-code
      (0 (ct:fail '#(expected_nonzero_exit)))
      (_
...
(defun x5_bad_import (config)
  (let* ((checker-bin (proplists:get_value 'checker_bin config))
         (fixture-dir (proplists:get_value 'fixture_dir config))
         (fixture (filename:join (list fixture-dir "cross_module" "bad_import.lfet")))
         (`#(,exit-code ,output) (run-checker-raw checker-bin fixture)))
    (case exit-code
      (0 (ct:fail '#(expected_nonzero_exit)))
      (_
