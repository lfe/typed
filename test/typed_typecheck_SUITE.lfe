...
(defun m3_3_well_typed_passes (config)
  (let* ((forms (check-and-decode config "typecheck/good" "well_typed.tlfe"))
         (priv-dir (proplists:get_value 'priv_dir config)))
    (case (typed_driver:compile_forms forms "well_typed.tlfe" priv-dir)
...
(defun m3_3_return_mismatch_rejected (config)
  (let* ((checker-bin (proplists:get_value 'checker_bin config))
         ...
         (priv-dir    (proplists:get_value 'priv_dir config))
         (fixture     (filename:join (list fixture-dir "typecheck" "bad" "return_mismatch.tlfe")))
...
(defun m3_4_arg_mismatch_rejected (config)
  (let* ((checker-bin (proplists:get_value 'checker_bin config))
         (fixture-dir (proplists:get_value 'fixture_dir config))
         (priv-dir    (proplists:get_value 'priv_dir config))
         (fixture     (filename:join (list fixture-dir "typecheck" "bad" "arg_mismatch.tlfe")))
